...
export class PieceViewer{
 ...
 display(data){
  for(const m of this.occluders)if(m.userData.proxy)m.geometry.dispose();this.occluders=[];for(const m of this.meshes.values()){m.geometry.dispose();m.material.dispose();this.scene.remove(m);}this.meshes.clear();this.pinLayer.replaceChildren();this.pins.clear();this.data=data;this.selected=null;
  for(const [name,bone] of Object.entries(data.bones)){const g=new T.BufferGeometry();g.setAttribute('position',new T.BufferAttribute(bone.position||new Float32Array(bone.vertices.flat()),3));g.setIndex(bone.index?new T.BufferAttribute(bone.index,1):bone.triangles.flat());g.computeVertexNormals();g.computeBoundingSphere();const m=new T.Mesh(g,new T.MeshStandardMaterial({color:0xece3cb,roughness:.7,side:T.DoubleSide}));this.meshes.set(name,m);this.scene.add(m);if(bone.occlusion){const proxy=new T.BufferGeometry();proxy.setAttribute('position',new T.BufferAttribute(bone.occlusion.position,3));proxy.setIndex(new T.BufferAttribute(bone.occlusion.index,1));proxy.computeBoundingSphere();const occluder=new T.Mesh(proxy,m.material);occluder.userData={proxy:true,bone:name};this.occluders.push(occluder);}else{m.userData.bone=name;this.occluders.push(m);}}
 ...
 frame(direction=this.direction,focus=false){
  ...
  const forward=new T.Vector3(...(this.data.viewDirections?.[direction]||directions[direction]));
 ...
 dispose(){for(const m of this.occluders)if(m.userData.proxy)m.geometry.dispose();this.observer.disconnect();this.controls.dispose();for(const m of this.meshes.values()){m.geometry.dispose();m.material.dispose();}this.renderer.dispose();this.renderer.domElement.remove();this.pinLayer.replaceChildren();}
}
