export class NotesStore {
 ...
 try{const r=await fetch('/api/notes?structure='+encodeURIComponent(id),{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({body,version:e.version})});const data=await r.json();if(r.status===409){e.conflict=data.note;throw new Error('Há uma versão diferente salva neste navegador. Escolha qual texto manter.');}if(!r.ok)throw new Error('Não foi possível salvar. Seu texto continua aqui para tentar novamente.');e.saved=body;e.version=data.version;return true;}catch(error){e.error=e.conflict?'Há uma versão diferente salva neste navegador. Escolha qual texto manter.':'Não foi possível salvar. Seu texto continua aqui para tentar novamente.';return false;}finally{e.saving=false;this.emit(id);}}
